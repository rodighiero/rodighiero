...
| `plugins` | the thirteen Ruby files in `_plugins/` — what each produces and who reads it, the naming scheme, the build-order rules |
...
**CSS** is fully inlined (no external stylesheets, to avoid render-blocking requests): `_includes/styles-base.css` holds the shared foundation (design tokens in `:root` — type scale, line heights, light/dark colors — plus base typography, `.card-meta`, `.authors`, `.cite-btn`, and the mode toggle) and `_includes/styles-font.css` the `@font-face` rules; both layouts `{% include %}` them at the top of their `<style>` block, and so does the standalone `404.html` for the fonts. The homepage's own rules are two more includes after them, `_includes/styles-gallery.css` and `_includes/styles-network.css`, so `home.html` itself holds only its `<head>` and markup. JS is inlined in the layouts too. The homepage's lives in `_includes/scripts-gallery.js` and `_includes/scripts-network.js`, plain JS files the layout includes into one `<script>` (so `node --check` works on them); shared snippets live in `_includes/site-scripts.html` (mode toggle, cite buttons), `_includes/site-head.html` (the page-invariant meta and link tags both `<head>`s share verbatim; the inventory is in the `seo` skill), `_includes/site-scheme.html` (the pre-paint color-scheme restore, which `404.html` needs as well) and `_includes/site-speculation.html` (the speculation rules that prefetch a link the reader is about to follow — prefetch only, never prerender, since a prerender would run `count.js` and log a pageview nobody made). There is no build step.
