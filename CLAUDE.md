...
**Network view** is a publication similarity graph rendered with D3 from `js/d3.v7.min.js` (lazy-loaded on first open). All similarity data is pre-computed offline by `scripts/build-network.py`, which: (i) parses Markdown abstracts and strips footnotes, headings, links, code, and bibliographies; (ii) machine-translates Italian-language abstracts to English via `Helsinki-NLP/opus-mt-it-en` (cached on disk in `_data/translations-cache.json` keyed by content hash); (iii) embeds title + abstract with `sentence-transformers/all-MiniLM-L6-v2`; (iv) computes pairwise cosine similarity; (v) extracts up to 8 KeyBERT keyphrases per paper (1–2 grams, MMR diversity 0.5) for the closest-matches sidebar; and (vi) writes `_data/network.json` (committed) with nodes, similarity matrix, and per-pair shared keyphrases. The page consumes that data via Liquid, runs 400 force-simulation ticks at render time so the layout appears already settled, then draws an edge from each node to its single strongest neighbour plus any pair scoring above `STRONG_SIM = 0.70`. Re-run `KMP_DUPLICATE_LIB_OK=TRUE python3 scripts/build-network.py` after editing publications.
