...
**Images** all live under `images/`: `images/publications/` holds the full-size card/hero images (the front-matter `img` values carry the `publications/` prefix); per-article figures sit in `images/<publication-slug>/` subfolders; `images/thumbnails/` mirrors the card images at max 800px width for the homepage (generated by `scripts/generate-thumbnails.py`, committed to the repo — rerun it after adding or changing a card image); the author portrait sits directly in `images/`. Publication pages, OG/Twitter cards, and JSON-LD use `/images/{{ img }}`; homepage cards use `/images/thumbnails/{{ img }}`.
