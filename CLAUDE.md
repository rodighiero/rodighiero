...
**Images** live in three root folders: `publications/` holds the full-size publication images (card/hero images at the top level, per-article figures in subfolders named after the publication slug); `thumbnails/` mirrors the card images at max 800px width for the homepage (generated by `scripts/generate-thumbnails.py`, committed to the repo — rerun it after adding or changing a card image); `images/` holds only site images (the author portrait). Publication pages, OG/Twitter cards, and JSON-LD use `/publications/…`; homepage cards use `/thumbnails/…`.
