...
# ── Homepage preview ──────────────────────────────────────────────────────────
# The gallery card shows a miniature of the graph, drawn from the same baked
# positions so it is always the map the network view renders. These are the live
# view's own values (see #network-view .link / .node .marker in home.html), so
# the card is a true reduction of the graph rather than a redrawn version of it.
...
PREVIEW_PATH = ROOT / "_includes" / "network-preview.svg"
PREVIEW_NODE_R = 3.0
...
def write_preview_svg(nodes: list[dict], links: list[dict]) -> None:
    """Write the homepage card's miniature of the graph.
    ...
    min_x, min_y = cx - half_w, cy - half_h
    w, h = half_w * 2, half_h * 2

    def pt(v: float) -> str:  # trim trailing zeros to keep the file small
    ...
    parts = [
        ...
        f'viewBox="0 0 {pt(w)} {pt(h)}" width="{pt(w)}" height="{pt(h)}" '
        f'fill="none" stroke="{PREVIEW_INK}" aria-hidden="true" focusable="false">',
        f'<g stroke-width="{PREVIEW_LINK_STROKE}" stroke-linecap="round">',
    ]
    for l in links:
        s, t = nodes[l["source"]], nodes[l["target"]]
        attrs = (
            f'x1="{pt(s["x"] - min_x)}" y1="{pt(s["y"] - min_y)}" '
            f'x2="{pt(t["x"] - min_x)}" y2="{pt(t["y"] - min_y)}"'
        )
    ...
    for n in nodes:
        parts.append(
            f'<circle cx="{pt(n["x"] - min_x)}" cy="{pt(n["y"] - min_y)}" r="{PREVIEW_NODE_R}"/>'
        )
    parts.append("</g></svg>")
...
def main() -> int:
    ...
    # The homepage card's miniature, from the very same positions written above.
    write_preview_svg(nodes, layout["links"])
