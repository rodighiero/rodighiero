...
def main() -> int:
    ...
    print("## Broken links — source context\n")
    for url, info in urls.items():
        print(f"### `[{info['status']}]` {url}\n")
        print(f"_{info['reason']}_\n")
        hits = occurrences.get(url, [])
        if not hits:
            print("_(not found in source tree — may be generated by a layout/include)_\n")
            continue
        for file, line_no, kind, ctx in hits:
            print(f"**`{file}:{line_no}`** — {kind}  ")
            print(f"> {ctx}\n")
    return 0
